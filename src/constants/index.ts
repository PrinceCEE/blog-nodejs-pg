import { ITableNames } from "../types";

export const COMMENT_TABLE_NAME: ITableNames = "comments";
export const POST_TABLE_NAME: ITableNames = "posts";
export const USER_TABLE_NAME: ITableNames = "users";
export const AUTH_TABLE_NAME: ITableNames = "auths";
export const FORGOT_PASSWORD_CODE_TABLE_NAME: ITableNames =
  "forgot_password_codes";

export const TOKEN_SECRET = "s$cret";
export const TOKEN_EXPIRES_IN = "7d";
